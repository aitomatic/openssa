...
class TestBaseSSM(unittest.TestCase):
    ...
    def test_discuss(self):
        ...
        self.base_ssm.custom_discuss.assert_called_with(
            user_input, self.base_ssm.get_conversation(conversation_id)
        )
        self.base_ssm.update_conversation.assert_called_with(
            user_input, None, conversation_id
        )
    ...
    def test_get_conversation(self):
        ...
        # pylint: disable=unsupported-assignment-operation
        self.base_ssm.conversations[conversation_id] = [
            {"role": "user", "content": "message"}
        ]
        self.assertEqual(
            self.base_ssm.get_conversation(conversation_id),
            [{"role": "user", "content": "message"}],
        )
    ...
    def test_select_methods(self):
        ...
        self.base_ssm.adapter.select_inferencers = MagicMock(
            return_value=["inferencer1"]
        )
    ...
    def do_not_test_conversation_history(self):
        self.base_ssm.reset_memory()
        ...
        expected_reply = {
            "role": "assistant",
            "content": "Hello, as the base implementation of SLM, this is all I can say.",
        }
        ...
        self.assertEqual(
            self.base_ssm.get_conversation(self.base_ssm.DEFAULT_CONVERSATION_ID),
            expected_conversation,
        )
